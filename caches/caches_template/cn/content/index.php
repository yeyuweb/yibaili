...
<div id="container" class="clearfix">
  <div class="wrapMain">
            <div class="homeLeft">
                 <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=7c8fbce75d5cc31266f5b06b5d97f000&sql=SELECT+ybl_cn_news.%2A+FROM+ybl_position_data+left+join+ybl_cn_news+on+ybl_position_data.id%3D+ybl_cn_news.id+where+ybl_position_data.posid%3D21+and+ybl_cn_news.id+is+not+null+++ORDER+BY+listorder+desc&num=1\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT ybl_cn_news.* FROM ybl_position_data left join ybl_cn_news on ybl_position_data.id= ybl_cn_news.id where ybl_position_data.posid=21 and ybl_cn_news.id is not null   ORDER BY listorder desc LIMIT 1");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
                                <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                ...
                <ul class="newsList clearfix">
                      <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=a821fab3cd10c0220e9525a174c69575&sql=SELECT+ybl_cn_news.%2A+FROM+ybl_position_data+left+join+ybl_cn_news+on+ybl_position_data.id%3D+ybl_cn_news.id+where+ybl_position_data.posid%3D21+and+ybl_cn_news.id+is+not+null+++ORDER+BY+listorder+desc&num=3\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT ybl_cn_news.* FROM ybl_position_data left join ybl_cn_news on ybl_position_data.id= ybl_cn_news.id where ybl_position_data.posid=21 and ybl_cn_news.id is not null   ORDER BY listorder desc LIMIT 3");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
                                <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                                <?php if($n!=1) { ?>
